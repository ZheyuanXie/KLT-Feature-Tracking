...
def objectTracking(rawVideo, draw_bb=False, play_realtime=False):
    ...
    for i in range(1,n_frame):
        ...
        for j in range(n_object):
            # for drawing the orginial shape of the bounding region (Yongyi Wang)
            # frames[i] = cv2.line(frames[i], (int(bboxs[i][j,0,0]), int(bboxs[i][j,0,1])), (int(bboxs[i][j,2,0]), int(bboxs[i][j,2,1])), (255, 0, 0), 2) 
            # frames[i] = cv2.line(frames[i], (int(bboxs[i][j,0,0]), int(bboxs[i][j,0,1])), (int(bboxs[i][j,1,0]), int(bboxs[i][j,1,1])), (255, 0, 0), 2) 
            # frames[i] = cv2.line(frames[i], (int(bboxs[i][j,1,0]), int(bboxs[i][j,1,1])), (int(bboxs[i][j,3,0]), int(bboxs[i][j,3,1])), (255, 0, 0), 2) 
            # frames[i] = cv2.line(frames[i], (int(bboxs[i][j,2,0]), int(bboxs[i][j,2,1])), (int(bboxs[i][j,3,0]), int(bboxs[i][j,3,1])), (255, 0, 0), 2) 
